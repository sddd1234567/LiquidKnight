...
#region Operations and Connection Methods
    ...
    protected internal void ExecuteRpc(Hashtable rpcData, int senderID = 0)
    {
        ...
        if (photonNetview == null)
        {
            ...
            if (owningPv)
            {
                Debug.LogWarning("Received RPC \"" + inMethodName + "\" for viewID " + netViewID + " but this PhotonView does not exist! View was/is ours." + (ownerSent ? " Owner called." : " endPoint called.") + " By: " + senderID);
            }
            else
            {
                Debug.LogWarning("Received RPC \"" + inMethodName + "\" for viewID " + netViewID + " but this PhotonView does not exist! Was remote PV." + (ownerSent ? " Owner called." : " endPoint called.") + " By: " + senderID + " Maybe GO was destroyed but RPC not cleaned up.");
            }
            return;
        }
